    ...
    def train(self, env):
        ...
        for i in tqdm(range(episodes)):
            ...
            while not done:
                ...
                if global_steps % update_rate == 0 and len(buffer) >= batch_size:
                    # gather a batch of experience data to train on
                    experiences = buffer.sample(batch_size)
                    states = torch.from_numpy(experiences[:, 0]).float().to(self._device)
                    actions = torch.from_numpy(experiences[:, 1]).float().to(self._device)
                    rewards = torch.from_numpy(experiences[:, 2]).float().to(self._device)
                    next_states = torch.from_numpy(experiences[:, 3]).float().to(self._device)
                    dones = torch.from_numpy(experiences[:, 4].astype(np.uint8)).float().to(self._device)
                    ...
                    q_targets_next = self._q_target(next_states).detach().max(1)[0].unsqueeze(1)
